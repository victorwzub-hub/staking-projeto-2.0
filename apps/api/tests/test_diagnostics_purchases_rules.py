...
TENANT_ID = UUID("10000000-0000-0000-0000-000000000401")
COMPANY_ID = UUID("20000000-0000-0000-0000-000000000401")
BRANCH_ID = UUID("30000000-0000-0000-0000-000000000401")
...
DIRECTIONAL_CHANGE_RULES: tuple[tuple[str, DirectionalOperator], ...] = (
    ("purchases.average_unit_cost_increase", "gt"),
    ("purchases.cancellation_rate_increase", "gt"),
    ("purchases.discount_rate_decline", "lt"),
    ("purchases.emergency_rate_increase", "gt"),
    ("purchases.freight_rate_increase", "gt"),
    ("purchases.freight_value_increase", "gt"),
    ("purchases.multiple_adherence_decline", "lt"),
    ("purchases.receipt_fill_rate_decline", "lt"),
    ("purchases.receipt_rate_decline", "lt"),
    ("purchases.return_rate_increase", "gt"),
)
...
CASES = (
    ...
    GoldenCase("purchases.average_unit_cost_increase", "12", "8", "previous", "10"),
    GoldenCase(
    ...
    GoldenCase("purchases.cancellation_rate_increase", "12", "8", "previous", "10"),
    GoldenCase(
    ...
    GoldenCase("purchases.discount_rate_decline", "8", "12", "previous", "10"),
    GoldenCase("purchases.emergency_rate_increase", "12", "8", "previous", "10"),
    GoldenCase("purchases.emergency_rate_persistent_positive", "1", "0", persisted=True),
    GoldenCase(
    ...
    GoldenCase("purchases.freight_rate_increase", "12", "8", "previous", "10"),
    GoldenCase("purchases.freight_value_increase", "12", "8", "previous", "10"),
    GoldenCase(
    ...
    GoldenCase("purchases.multiple_adherence_decline", "8", "12", "previous", "10"),
    GoldenCase(
    ...
    GoldenCase("purchases.receipt_fill_rate_decline", "8", "12", "previous", "10"),
    GoldenCase(
    ...
    GoldenCase("purchases.receipt_rate_decline", "8", "12", "previous", "10"),
    GoldenCase(
    ...
    GoldenCase("purchases.return_rate_increase", "12", "8", "previous", "10"),
    GoldenCase("purchases.return_rate_positive", "1", "0"),
)
...
def _observation(
    ...
    *,
    ...
    return KPIObservation(
        ...
        data_version=24,
        formula_version=1,
        lineage_ref=f"lineage://purchases/{kpi_code.replace('.', '/')}/{kind}",
...
def test_purchases_catalog_has_twenty_real_ordered_unique_rules() -> None:
    assert len(PURCHASES_RULES) == 20
    assert [rule.code for rule in PURCHASES_RULES] == sorted(rule.code for rule in PURCHASES_RULES)
    assert all(rule.domain == "purchases" for rule in PURCHASES_RULES)
    assert all(rule.status == "active" for rule in PURCHASES_RULES)
    assert len({rule.code for rule in PURCHASES_RULES}) == 20
    assert len({rule.rule_definition_id for rule in PURCHASES_RULES}) == 20
    assert len({rule.to_snapshot().condition_hash for rule in PURCHASES_RULES}) == 20
    assert len({rule.to_snapshot().definition_hash for rule in PURCHASES_RULES}) == 20
    assert len({rule.governance_hash for rule in PURCHASES_RULES}) == 20


def test_purchases_rules_use_only_operational_kpis_and_exact_safe_actions() -> None:
    unavailable = {item.code for item in UNAVAILABLE_KPIS}
    for rule in PURCHASES_RULES:
        validate_rule(rule)
        assert rule.primary_kpi_code in KPI_BY_CODE
        assert KPI_BY_CODE[rule.primary_kpi_code].category == "purchases"
        ...
        for reference in rule.actions:
            ...
            assert action.domain == "purchases"
...
def test_purchases_golden_cases_match_and_do_not_match(case: GoldenCase) -> None:
...
@pytest.mark.parametrize("case", CASES, ids=lambda case: case.rule_code)
def test_purchases_evidence_is_factual_and_hypotheses_are_separate(case: GoldenCase) -> None:
    rule = RULE_BY_CODE[case.rule_code]
...
@pytest.mark.parametrize("case", CASES, ids=lambda case: case.rule_code)
def test_purchases_recommendations_remain_advisory_only(case: GoldenCase) -> None:
    result = evaluate_rule(_evaluation(case, matched=True))
    assert result.state == "matched"
...
def test_persistent_emergency_rate_requires_two_history_frames() -> None:
...
def test_persistent_emergency_rate_does_not_match_after_current_recovery() -> None:
    case = next(item for item in CASES if item.persisted)
...
def test_purchases_manifest_is_deeply_immutable() -> None:
    first = catalog_manifest(PURCHASES_RULES)[0]
    ...
    with pytest.raises(TypeError):
        first["code"] = "purchases.changed"  # type: ignore[index]


def test_purchases_slice_and_global_catalog_hashes_are_explicit_and_preserved() -> None:
...
def test_purchases_catalog_is_stable_across_python_hash_seeds() -> None:
...
print(json.dumps({
    ...
    "slice_hash": catalog_hash(PURCHASES_RULES),
...
"""
    ...
    assert payload["slice_hash"] == EXPECTED_PURCHASES_HASH
    assert payload["global_hash"] == EXPECTED_GLOBAL_HASH


def test_purchases_rule_module_has_no_infrastructure_imports_or_pct_direction_test() -> None:
    path = (
