...
TENANT_ID = UUID("10000000-0000-0000-0000-000000000501")
COMPANY_ID = UUID("20000000-0000-0000-0000-000000000501")
BRANCH_ID = UUID("30000000-0000-0000-0000-000000000501")
...
DIRECTIONAL_CHANGE_RULES: tuple[tuple[str, DirectionalOperator], ...] = (
    ("suppliers.average_lead_time_increase", "gt"),
    ("suppliers.average_minimum_order_increase", "gt"),
    ("suppliers.failure_rate_increase", "gt"),
    ("suppliers.fill_rate_decline", "lt"),
    ("suppliers.on_time_rate_decline", "lt"),
    ("suppliers.quality_score_decline", "lt"),
    ("suppliers.return_rate_increase", "gt"),
)
...
CASES = (
    GoldenCase("suppliers.average_lead_time_above_network", "12", "8", "network_average", "10"),
    GoldenCase("suppliers.average_lead_time_increase", "12", "8", "previous", "10"),
    GoldenCase("suppliers.average_minimum_order_increase", "12", "8", "previous", "10"),
    GoldenCase("suppliers.cost_variation_above_network", "12", "8", "network_average", "10"),
    GoldenCase("suppliers.cost_variation_positive", "1", "0"),
    GoldenCase("suppliers.dependency_above_network", "12", "8", "network_average", "10"),
    GoldenCase("suppliers.failure_rate_increase", "12", "8", "previous", "10"),
    GoldenCase("suppliers.failure_rate_persistent_positive", "1", "0", persisted=True),
    GoldenCase("suppliers.failure_rate_positive", "1", "0"),
    GoldenCase("suppliers.fill_rate_below_network", "8", "12", "network_average", "10"),
    GoldenCase("suppliers.fill_rate_decline", "8", "12", "previous", "10"),
    GoldenCase("suppliers.on_time_rate_below_network", "8", "12", "network_average", "10"),
    GoldenCase("suppliers.on_time_rate_decline", "8", "12", "previous", "10"),
    GoldenCase("suppliers.quality_score_below_network", "8", "12", "network_average", "10"),
    GoldenCase("suppliers.quality_score_decline", "8", "12", "previous", "10"),
    GoldenCase("suppliers.return_rate_above_network", "12", "8", "network_average", "10"),
    GoldenCase("suppliers.return_rate_increase", "12", "8", "previous", "10"),
    GoldenCase("suppliers.stockout_association_above_network", "12", "8", "network_average", "10"),
    GoldenCase("suppliers.stockout_association_positive", "1", "0"),
    GoldenCase("suppliers.top5_concentration_above_network", "12", "8", "network_average", "10"),
)
...
def _observation(
    ...
    *,
    ...
    return KPIObservation(
        ...
        data_version=25,
        formula_version=1,
        lineage_ref=f"lineage://suppliers/{kpi_code.replace('.', '/')}/{kind}",
...
def test_suppliers_catalog_has_twenty_real_ordered_unique_rules() -> None:
    assert len(SUPPLIERS_RULES) == 20
    assert [rule.code for rule in SUPPLIERS_RULES] == sorted(rule.code for rule in SUPPLIERS_RULES)
    assert all(rule.domain == "suppliers" for rule in SUPPLIERS_RULES)
    assert all(rule.status == "active" for rule in SUPPLIERS_RULES)
    assert len({rule.code for rule in SUPPLIERS_RULES}) == 20
    assert len({rule.rule_definition_id for rule in SUPPLIERS_RULES}) == 20
    assert len({rule.to_snapshot().condition_hash for rule in SUPPLIERS_RULES}) == 20
    assert len({rule.to_snapshot().definition_hash for rule in SUPPLIERS_RULES}) == 20
    assert len({rule.governance_hash for rule in SUPPLIERS_RULES}) == 20


def test_suppliers_rules_use_only_operational_kpis_and_exact_safe_actions() -> None:
    unavailable = {item.code for item in UNAVAILABLE_KPIS}
    for rule in SUPPLIERS_RULES:
        validate_rule(rule)
        assert rule.primary_kpi_code in KPI_BY_CODE
        assert KPI_BY_CODE[rule.primary_kpi_code].category == "suppliers"
        ...
        for reference in rule.actions:
            ...
            assert action.domain == "suppliers"
...
def test_suppliers_golden_cases_match_and_do_not_match(case: GoldenCase) -> None:
...
@pytest.mark.parametrize("case", CASES, ids=lambda case: case.rule_code)
def test_suppliers_evidence_is_factual_and_hypotheses_are_separate(case: GoldenCase) -> None:
    rule = RULE_BY_CODE[case.rule_code]
...
@pytest.mark.parametrize("case", CASES, ids=lambda case: case.rule_code)
def test_suppliers_recommendations_remain_advisory_only(case: GoldenCase) -> None:
    result = evaluate_rule(_evaluation(case, matched=True))
    assert result.state == "matched"
...
def test_persistent_failure_rate_requires_two_history_frames() -> None:
...
def test_persistent_failure_rate_does_not_match_after_current_recovery() -> None:
    case = next(item for item in CASES if item.persisted)
...
def test_suppliers_manifest_is_deeply_immutable() -> None:
    first = catalog_manifest(SUPPLIERS_RULES)[0]
    ...
    with pytest.raises(TypeError):
        first["code"] = "suppliers.changed"  # type: ignore[index]


def test_suppliers_slice_and_global_catalog_hashes_are_explicit_and_preserved() -> None:
...
def test_suppliers_catalog_is_stable_across_python_hash_seeds() -> None:
...
print(json.dumps({
    ...
    "slice_hash": catalog_hash(SUPPLIERS_RULES),
...
"""
    ...
    assert payload["slice_hash"] == EXPECTED_SUPPLIERS_HASH
    assert payload["global_hash"] == EXPECTED_GLOBAL_HASH


def test_suppliers_rule_module_has_no_infrastructure_imports_or_pct_direction_test() -> None:
    path = (
